...
Vvveb.Gui = {
	...
	async deployToNetlify() {
		// 1) Get the HTML content generated by the user
		const htmlContent = Vvveb.Builder.getHtml();
	
		// 2) Prompt the user for their Netlify Personal Access Token, Site ID (or leave blank for a new site), and Project Name
		const netlifyToken = prompt("Please enter your Netlify Personal Access Token:");
		const netlifyClientSiteID = prompt("Please enter your Netlify Site ID (leave blank to create a new site):");
		const projectName = prompt("Please enter the name for your Netlify project:");
	
		// 3) Create a ZIP file with the HTML content
		const zipFile = new JSZip();
		zipFile.file("index", htmlContent, { base64: true, binary: true });
		const zipContent = await zipFile.generateAsync({ type: "uint8array" });
	
		// 4) Call the Netlify API to deploy the website
		try {
			let response;
			const url = netlifyClientSiteID
				? `https://api.netlify.com/api/v1/sites/${netlifyClientSiteID}/deploys`
			...
			if (response.ok) {
				...
				const siteUrl = netlifyClientSiteID
					? `https://${data.subdomain}.netlify.app`
					: `https://${data.site_url}`;
				prompt(`Site deployed successfully: ${siteUrl}`);
			} else {
				prompt(`Feailed to deploy`);
